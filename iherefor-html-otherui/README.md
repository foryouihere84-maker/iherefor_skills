...
为准；输出是目标技术栈的生产代码，
默认经过「约束闭合检查 → 源码规范检查」即可交给人工体验调整；编译、多尺寸几何验证和视觉审查
是按需增强验证，不作为每次生成 UI 代码的固定成本。
设计稿中的宽高是参考事实，不等于生产约束 —— **闭合契约**要求每条尺寸都写清「由什么闭合」
（尺寸轴：`fixed` / `intrinsic` / `bounded` / `aspect-ratio`），每条位置都相对直接父视图表达
（关系轴：`pinned` / `proportional` / `equal` / `centered`），不能把所有组件尺寸按探针设备的值固定。
`fixed` 不是默认值，只留给真正的视觉常量（图标、装饰、边框、明确固定高度的控件），且必须给出理由。
