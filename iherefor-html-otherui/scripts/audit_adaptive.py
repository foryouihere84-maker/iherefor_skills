...
**它不做像素比对，这是刻意的。** Lanhu 只提供一份设计稿，``reference/reference.png``
是某一台设备的像素基准；拿它去比 iPad 截图是拿两个不同画布比对，
``audit_alignment.py`` 的 ``domVsReference`` 会直接判「基准不可信」。这不是容差不够大，
是**证据类型不匹配**。所以平板这一关断言的是**几何关系**，输入是各采样的几何转储
（``actual/geometry-<sample-id>.json``），不是截图。
